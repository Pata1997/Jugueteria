"""
Generador de tickets para Notas de Débito
...
def generar_nota_debito_ticket_pdf(nota):
    """
    Genera PDF para Nota de Débito cumpliendo con requisitos SET Paraguay
    Muestra: Factura Original, Motivo, Timbrado Vence, Nuevo Total a Pagar
    """
    print(f"[nota_debito_pdf] Generando PDF para ND {nota.id}", file=sys.stderr, flush=True)
    ...
    try:
        ...
        if logo_path and os.path.exists(logo_path):
            try:
                ...
                print(f"[nota_debito_pdf] Error al cargar logo: {e}", file=sys.stderr, flush=True)
        ...
        # Número de ND destacado con formato completo
        c.setFont("Helvetica-Bold", 10)
        # Extraer solo el número de la ND (ej: 0000007 de ND-0000007)
        numero_nd_base = nota.numero_nota.replace('ND-', '').lstrip('0').zfill(7)
        numero_nd_completo = f"ND {establecimiento_punto}-{numero_nd_base}"
        c.drawString(x_margin, y, f"N°: {numero_nd_completo}")
        y -= 4 * mm
        ...
        motivo_texto = nota.motivo or "No especificado"
            ...
            if hasattr(nota, 'pagos') and nota.pagos:
                for pago in nota.pagos:
                    if pago.estado == 'confirmado':
                        ...
                        monto_pago = f"{int(pago.monto):,}".replace(',', '.')
        ...
        c.drawString(x_margin, y, f"Emisión: {nota.estado_emision or 'N/A'}")
        y -= 3 * mm
        ...
        print("[nota_debito_pdf] Dibujado completado, guardando PDF", file=sys.stderr, flush=True)
        c.save()
        ...
        print(f"[nota_debito_pdf] PDF generado: {len(pdf_bytes)} bytes", file=sys.stderr, flush=True)
        ...
        print(f"[nota_debito_pdf] EXCEPCIÓN: {type(e).__name__}: {str(e)}", file=sys.stderr, flush=True)
